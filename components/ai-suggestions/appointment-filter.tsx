...
export function AppointmentFilter({
    ...
    const toggleAppointment = (appointment: Appointment) => {
        if (selectedAppointments.some((a) => a.id === appointment.id)) {
            onSelectAppointments(selectedAppointments.filter((a) => a.id !== appointment.id))
        } else {
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="p-4 border-b">
                ...
                <p className="text-sm text-muted-foreground">Select appointments to include in the AI assessment</p>
            </div>
            ...
            {selectedAppointments.length > 0 && (
                <div className="p-4 border-b bg-muted/50">
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="font-medium">Selected Appointments ({selectedAppointments.length})</h3>
                        <Button variant="ghost" size="sm" onClick={() => onSelectAppointments([])}>
                            Clear all
                        </Button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {selectedAppointments.map((appointment) => (
                            <Badge key={appointment.id} variant="secondary" className="flex items-center gap-1">
                                {new Date(appointment.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })} - {appointment.reason}
                                <Button
                                    variant="ghost"
                                    size="sm"
